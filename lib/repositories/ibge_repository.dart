...
class IBGERepository {
  Future<List<UF>> getUFListFromApi() async {
    ...
    if (preferences.containsKey('UF_LIST')) {
      final jp = json.decode(preferences.get('UF_LIST'));
      return jp.map<UF>((j) => UF.fromJson(j)).toList()
  ...
  Future<List<City>> getCityListFromApi(UF uf) async {
    ...
    try {
      ...
      return response.data.map<City>((j) => City.fromJson(j)).toList()
        ..sort((a, b) => a.name.toLowerCase().compareTo(b.name.toLowerCase()));
    } on DioError {
      return Future.error('Falha ao obter lista de Cidades do Estado ${uf.id}');
    }
  }
}
